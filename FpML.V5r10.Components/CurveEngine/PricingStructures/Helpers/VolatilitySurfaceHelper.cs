...
namespace Orion.CurveEngine.PricingStructures.Helpers
{
    ...
    public class VolatilitySurfaceHelper
    {
        ...
        public static decimal[,] GetExpirationByStikeVolatilityMatrix(IEnumerable<PricingStructurePoint> dataPoints)
        {
            ...
            int x = 0;
            foreach (TimeDimension expiraitionCoordinate in GetDimensionValues(pricingStructurePoints, CubeDimension.Expiration))
            {
                int y = 0;
                foreach (decimal strikeCoordinate in GetDimensionValues(pricingStructurePoints, CubeDimension.Strike))
                {
                    result[x, y] = GetValueByExpirationAndStrike(pricingStructurePoints, expiraitionCoordinate, strikeCoordinate);
                    ++y;
                }
                ++x;
            }
        ...
        ///<returns></returns>
        ///<exception cref="ArgumentOutOfRangeException"></exception>
        public static IEnumerable GetDimensionValues(IEnumerable<PricingStructurePoint> dataPoints, CubeDimension dimension)
        {
            var strikeList = new List<Decimal>();
            var expirationList = new List<TimeDimension>();
            var termList = new List<TimeDimension>();
