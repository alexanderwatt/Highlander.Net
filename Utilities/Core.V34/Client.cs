...
namespace Core.V34
{
    internal class ClientItem : CommonItem, ICoreItem
    {
        ...
        public void Freeze()
        {
            ...
            if (xtki != null)
            {
                _xData = _proxy.CryptoManager.EncryptWithTranspKey(xtki, _zData);
        ...
        private void Decrypt()
        {
            if (_zData == null)
            {
                ...
                var xtki = SysProps.GetValue<string>(SysPropName.XTKI, null);
                if (xAlg > 0 && xtki != null)
                    _zData = _proxy.CryptoManager.DecryptWithTranspKey(xtki, _xData);
                else
                    _zData = _xData;
                _xData = null;
            }
    ...
    internal class CoreClient : ICoreClient, ITransferV341
    {
        ...
        private readonly IModuleInfo _clientInfo;
        ...
        private SerialFormat _defaultSerialFormat = SerialFormat.Undefined;

        /// <summary>
        /// </summary>
        public SerialFormat DefaultSerialFormat { get => _defaultSerialFormat;
            set => _defaultSerialFormat = value;
        }
        ...
        /// <summary>
        /// </summary>
        public CoreModeEnum CoreMode { get; } = CoreModeEnum.Standard;
        ...
        public TimeSpan RequestTimeout
        {
            ...
            set
            {
                ...
                _logger.LogDebug("New request timeout is: {0}", value);
            }
        }
        ...
        public TimeSpan OfflineTimeout
        {
            ...
            set
            {
                ...
                _logger.LogDebug("New offline timeout is: {0}", value);
            }
        }
        ...
        public int MaxRequestCount
        {
            ...
            set
            {
                ...
                _logger.LogDebug("New max request count is: {0}", value);
            }
        }
        ...
        private void EnqueueOutgoingRequest(RequestBase request)
        {
            ...
            while (requestCount > _maxRequestCount)
            {
                ...
                {
                    _logger.LogDebug("Request '{0}' blocked until request count <= {1}", request.RequestId, _maxRequestCount);
            ...
            if (logged)
                _logger.LogDebug("Request '{0}' unblocked.", request.RequestId);
            ...
            Assembly applAssembly)
        {
            ...
            _mainThreadQueue = new AsyncThreadQueue(_logger);
            ...
            catch (PrincipalException principalException)
            {
                ...
                _logger.LogWarning("UserPrincipal.FindByIdentity failed. User name: {0}. {1}: {2}", userLoginName, principalException.GetType().Name, principalException.Message);
            }
            catch (COMException e)
            {
                // Can occur when trying to run as a scheduled task
                _logger.LogWarning("UserPrincipal.FindByIdentity COM failure: User name: {0}. {1}: {2}", userLoginName, e.GetType().Name, e.Message);
            }
            Assembly coreAssembly = Assembly.GetExecutingAssembly();
            _clientInfo = new ModuleInfo(env, clientId, userLoginName, userFullName, applAssembly, coreAssembly);
            ...
            ServiceAddress resolvedServer = V111Helpers.ResolveServer(_logger, svcName, new ServiceAddresses(_protocols, serviceAddress, defaultPort),
            ...
            _logger.LogInfo("Client connected to: {0}", _transferAddressBinding.Address.Uri.AbsoluteUri);
        }
        ...
        public void SafeDisconnect()
        {
            ...
            {
                _logger.LogWarning("Close session attempt failed: {0}: {1}", wcfExcp.GetType().Name, wcfExcp.Message);
            }
            ...
            _subscriptions.Locked(subscriptions =>
            {
                if (subscriptions.TryGetValue(subscriptionId, out var subscription))
                {
                    ...
                    try
                    {
                        ...
                        _logger.LogDebug("Cancelling subscription: <{0}> {1} ({2})", subscription.DataTypeName, subscription.WhereExpr.DisplayString(), subscription.Id);
                    }
                    catch (Exception e)
                    {
                        _logger.LogError("Failed to cancel subscription: {0}: {1}", e.GetType().Name, e.Message);
                    }
                }
            });
        ...
        public void UnsubscribeAll()
        {
            ...
            if (arList.Count > 0)
            {
                ...
                _logger.LogDebug("Cancelled {0} subscriptions.", arList.Count);
            }
        }
        ...
        {
            var item = new ClientItem(this, ItemKind.Debug, _defaultAppScopes[0], true, name, props, data, typeof(T), _defaultSerialFormat,
            ...
            ThreadPool.QueueUserWorkItem(state =>
            {
                ...
                {
                    _logger.LogError("SaveDebug completion failed: {0}", e.GetType().Name);
                }
            }, ar);
        }
        ...
        private ClientItem ConvertToClientItem(ICoreItem item)
        {
            var result = item as ClientItem ??
                         new ClientItem(this, item.ItemKind, item.AppScope, item.Transient, item.Name, item.AppProps, item.Data, item.DataType, _defaultSerialFormat, item.Lifetime);
            return result;
        }
        ...
        public ICoreItem MakeTypedItem(Type dataType, object data, string name, NamedValueSet props, bool transient)
        {
            ...
            return new ClientItem(this, ItemKind.Object, _defaultAppScopes[0], transient, name, props, data, dataType, _defaultSerialFormat,
        ...
        private IEnumerable<ClientItem> EndRequest(IAsyncResult ar)
        {
            ...
            if (debugRequests)
                _logger.LogDebug("Waiting for completion...");
        ...
        private void HeartbeatTimeout(object notUsed)
        {
            ...
            try
            {
                ...
                string itemName =
                    $"System.Heartbeat.{_clientInfo.HostName}.{_clientInfo.UserName}.{_clientInfo.ApplName}.{_clientInfo.NodeGuid}";
                ICoreItem item = MakeItem("Heartbeat", itemName, null, true, TimeSpan.FromMinutes(10));
                SaveItem(item);
            }
            catch (Exception e)
            {
                _logger.LogError("Heartbeat failed: {0}: {1}", e.GetType().Name, e.Message);
            }
            finally
            {
        ...
        private void CommenceHeaderProcessing(V131SessionHeader header, out RequestBase request)
        {
            ...
            {
                // unknown request
                _logger.LogWarning("Request '{0}' unknown - ignoring data", header.RequestId);
            }
        ...
        private void CompleteHeaderProcessing(V131SessionHeader header, RequestBase request, Exception excp)
        {
            if (request != null)
            {
                if (excp != null)
                {
                    ...
                    AsyncCallProcessOutgoingRequests();
                    _logger.LogError("Request '{0}' failed: {1}", header.RequestId, excp);
                }
                else if (!header.MoreFollowing)
                {
                    ...
                    if (header.DebugRequest)
                        _logger.LogDebug("Request '{0}' completed normally ({1}ms).", header.RequestId, (DateTimeOffset.Now - request.SubmitTime).TotalMilliseconds);
                }
            }
        }
        ...
        private void PerformSyncRecvAnswerMultipleItems(PackageAnswerMultipleItems package)
        {
            ...
            try
            {
                if (DebugRequests || package.Header.DebugRequest)
                    _logger.LogDebug("Request '{0}' received {1} items",
        ...
        private void PerformSyncRecvNotifyMultipleItems(PackageNotifyMultipleItems package)
        {
            ...
            if (DebugRequests || package.Header.DebugRequest)
            {
                ...
                {
                    _logger.LogDebug("Subscription '{0}' received {1} items",
            ...
            if (subscription != null)
            {
                ...
                if (package.Body.Items != null)
                    ...
                    {
                        //if (_DebugRequests || package.Header.DebugRequest)
                        //    _Logger.LogDebug("Subscription '{0}' received item: {1}", package.Body.SubscriptionId, item.ItemName);
            ...
            else
            {
                ...
                _logger.LogWarning("Subscription '{0}' unknown - ignoring data", package.Body.SubscriptionId);
            }
        }
        ...
        private void PerformSyncRecvCompletionResult(PackageCompletionResult package)
        {
            ...
            {
                if (DebugRequests || package.Header.DebugRequest)
                    _logger.LogDebug("Request '{0}' received CompletionResult", package.Header.RequestId);
                CommenceHeaderProcessing(package.Header, out request);
                if (request != null)
                {
            ...
            while (_replyServiceHost == null)
            {
                ...
                try
                {
                    ...
                    _replyServiceHost = new CustomServiceHost<ITransferV341, TransferRecverV341>(
                        _logger, new TransferRecverV341(this), endpoint,
                        _clientInfo.ApplName, typeof(ITransferV341).Name, false);
                }
                catch (AddressAlreadyInUseException e1)
                {
                    // expected often
                    _logger.LogDebug("Failed to open port {0}: {1}: {2}", port, e1.GetType().Name, e1.Message);
                    DisposeHelper.SafeDispose(ref _replyServiceHost);
                }
                catch (InvalidOperationException e2)
                {
                    // expected but not often
                    _logger.LogDebug("Failed to open port {0}: {1}: {2}", port, e2.GetType().Name, e2.Message);
                    DisposeHelper.SafeDispose(ref _replyServiceHost);
                }
                catch (CommunicationException e3)
                {
                    // unexpected communications error
                    _logger.LogWarning("Failed to open port {0}: {1}: {2}", port, e3.GetType().Name, e3.Message);
                    DisposeHelper.SafeDispose(ref _replyServiceHost);
                    throw;
                }
            } // while
        ...
        private void BackoffRetryAlgorithm(Exception excp, Guid requestId, DateTimeOffset expiryTime, RequestBase request, int attempt)
        {
            ...
            {
                // initial catch
                _logger.LogWarning("Request '{0}' to server at '{1}' failed: {2}",
                    requestId, _transferAddressBinding.Address.Uri.AbsoluteUri, excp.GetType().Name);
                //_Logger.LogDebug("Request '{0}': connection attempts will continue until {1}", requestId, expiryTime);
            }
            else
            {
                // repeat catch
                _logger.LogDebug("Request '{0}': connection attempt ({1}) failed: {2}",
            ...
            try
            {
                ...
                while (!sent && _coreState != CoreStateEnum.Faulted)
                {
                    ...
                    try
                    {
                        ...
                        {
                            //if (attempt > 1)
                            //    _Logger.LogDebug("Reconnect attempt ({0}) to server at: {1}", attempt, _SessCtrlAddressBinding.Address.Uri.AbsoluteUri);
                        ...
                        if (request != null)
                        {
                            if (request.DebugRequest)
                                _logger.LogDebug("Request '{0}': sending {1} ...", requestId, request.GetType().Name);
                            ...
                            request.Transmit(_logger, _clientBase, header);
                        }
                        sent = true;
                        if ((request != null) && (request.DebugRequest))
                            _logger.LogDebug("Request '{0}': sent.", requestId);
                        if (attempt > 1)
                            _logger.LogDebug("Request '{0}': Connect attempt ({1}) succeeded.", requestId, attempt);
                        // end outage (if any)
        ...
        private void AsyncExecProcessOutgoingRequests(object notUsed1)
        {
            ...
            {
                _logger.LogError("HousekeepingRequests: Unhandled exception: {0}", e);
            }
        ...
        {
            SaveAppSettings(settings, _clientInfo.ApplName, _clientInfo.UserName, _clientInfo.HostName, false, _clientInfo.ConfigEnv);
        }
        ...
        {
            SaveAppSettings(settings, applName, _clientInfo.UserName, _clientInfo.HostName, false, _clientInfo.ConfigEnv);
        }
        ...
        public NamedValueSet LoadAppSettings(string applName, string userName, string hostName)
        {
            ...
            //result.LogValues((text) => _Logger.LogDebug("  " + text));
            return result;
        }
        ...
        {
            var result = new Subscription(_logger, this);
        ...
        internal void StartSubscription(Guid subscriptionId)
        {
            ...
            _subscriptions.Locked(subscriptions =>
            {
                ...
                if (subscription.WhereExpr != null)
                    _logger.LogDebug("Started subscription: <{0}> {1} ({2})", subscription.DataTypeName,
            ...
            _subscriptions.Locked(subscriptions =>
            {
                if (subscriptions.TryGetValue(subscriptionId, out var subscription))
                {
                    ...
                    if (subscription.Started)
                    {
                        ...
                        _logger.LogDebug("Subscription '{0}' cancelled", subscription.Id);
                    }
                    else
                    {
                        _logger.LogDebug("Subscription '{0}' not started!", subscription.Id);
                    }
                }
                else
                {
                    _logger.LogDebug("Subscription '{0}' missing!", subscription.Id);
                }
            });
            return ar;
        }
        ...
        {
            return new CoreCache(_logger, this, null, null);
        }
        ...
        {
            return new CoreCache(_logger, this, changeHandler, syncContext);
        }
