...
namespace National.QRSC.Runtime.V31
{
    /// <summary>
    ...
    public interface ICoreClient : IDisposable
    {
        ...
        [Obsolete("Use MakeObject<T>(T data,...) where possible", false)]
        ICoreItem MakeObject(object data, string name, NamedValueSet props);
        [Obsolete("Use MakeObject<T>(T data,...) where possible", false)]
        ICoreItem MakeObject(string dataTypeName, string text, string name, NamedValueSet props);
        // save methods
        ...
        [Obsolete("Use SaveObject<T>(T data,...) where possible", false)]
        Guid SaveObject(object data, string name, NamedValueSet props);
        [Obsolete("Use SaveObject<T>(T data,...) where possible", false)]
        Guid SaveObject(object data, string name, NamedValueSet props, TimeSpan lifetime);
        ...
        [Obsolete("Use LoadItems<T>(IExpression whereExpr) where possible.", false)]
        List<ICoreItem> LoadItems(string dataTypeName, IExpression whereExpr);
    ...
    public interface ICoreClientEx : ICoreClient
    {
        ...
        // - client info
        //ModuleInfo ClientInfo { get; }

        // - raw items
        IAsyncResult SaveRawItemBegin(V31StoreItem storeItem);
        void SaveRawItemEnd(IAsyncResult ar);
        void SaveRawItem(V31StoreItem storeItem);
        void SaveRawItems(IEnumerable<V31StoreItem> storeItems);
    ...
    public class V31StoreItem
    {
        ...
        // constructors
        public V31StoreItem(ICoreItem item, string newItemName)
        {
        ...
        public V31StoreItem(ICoreItem item) : this(item, null) { }
    }
