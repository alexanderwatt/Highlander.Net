...
namespace Orion.WindowsForms.ChartControl
{
    ...
    public class Chart : Control
    {
        ...
        private const float LegendMaxWidth = 0.25F; // max percents that legend can strech
        ...
        protected virtual void OnDrawChartLegend(DrawChartEventArgs e)
        {
            ...
            // check if there is text to draw
            if (e.Cancel == false && this._legend.Visible)
            {
                // initialize graphic resources
                Pen borderPen = this._legend.Border.ToPen();
                Brush textBrush = new SolidBrush(this._series.Title.Color);
                Pen linePen = this._series.Line.ToPen();
                Pen markBorderPen = new Pen(this._series.Mark.BorderColor);
                Brush markFillPen = new SolidBrush(this._series.Mark.FillColor);
                try
                {
                    ...
                    int borderWidth = (int)Math.Ceiling(this._legend.Border.Weight / 2);
                    borderWidth = borderWidth > 0 ? borderWidth - 1 : 0;

                    Point lineCap = e.Bounds.Location;
                    lineCap.Offset(borderWidth, borderWidth);
                    lineCap.Offset(Chart.LegendMargin, Chart.LegendMargin);
                    lineCap.Offset(0, Math.Max(itemSize.Height / 2, Chart.MarkSize));

                    Point lineCap2 = lineCap;
                    lineCap2.Offset(Chart.MarkSize * 4, 0);

                    // draw sample series line & mark
                    e.Graphics.DrawLine(linePen, lineCap, lineCap2);
                    lineCap.Offset(Chart.MarkSize * 2, 0);
                    this.DoDrawChartMark(e.Graphics, markBorderPen, markFillPen, lineCap, this._series.Mark.Shape, Chart.MarkSize);
                    lineCap2.Offset(Chart.LegendMargin, 0);

                    // draw series title
                    Rectangle itemRect = new Rectangle(lineCap2, new Size(itemSize.Width + Chart.LegendMargin * 2, 0));
                    e.Graphics.DrawString(this._series.Title.Text, this._series.Title.Font, textBrush, itemRect, format);

                    // check if we need to draw the lened's border
                    if (this._legend.Border.Visible && this._legend.Border.Weight > 0)
                    {
        ...
        protected virtual void OnDrawChartXAxisTitle(DrawChartEventArgs e)
        {
            ...
            // check if there is text to draw
            if (e.Cancel == false && this._xAxis.Title.Text.Length != 0)
            {
                // initialize graphic resources
                Brush textBrush = new SolidBrush(this._xAxis.Title.Color);
                try
                {
                    // draw the time axis title
                    StringFormat format = CustomGraphics.StringFormat(ContentAlignment.MiddleCenter);
                    e.Graphics.DrawString(this._xAxis.Title.Text, this._xAxis.Title.Font, textBrush, e.Bounds, format);
                }
                finally
                {
        ...
        protected virtual void OnDrawChartXAxis(DrawChartEventArgs e)
        {
            ...
            if (e.Cancel == false && this._xAxis.Line.Visible && this._xAxis.Line.Weight > 0)
            {
                ...
                // initialize graphic resources
                Pen linePen = this._xAxis.Line.ToPen();
                Brush labelBrush = new SolidBrush(this._xAxis.Labels.Color);
                try
                {
                    // draw the axis line
                    ...
                    DateTime minDate = this._xAxis.Scale.Minimum;
                    DateTime maxDate = this._xAxis.Scale.Maximum;
                    TimeSpan dateDiff = (maxDate - minDate);
                    int min = 0;
                    int max = dateDiff.Days;
                    int length = end.X - start.X;
                    int step = this._xAxis.Scale.MajorUnit;
                    ...
                    if (step > 0 && max > min)
                    {
                        ...
                        for (int tick = min; tick <= max; tick += step)
                        {
                            ...
                            this.DoDrawAxisTick(e.Graphics,
                                                linePen,
                                                tickCap,
                                                this._xAxis.MajorTick,
                                                true,
                                                false,
                                                this._xAxis.Labels.Visible ? tickLabel : null,
                                                this._xAxis.Labels.Rotation,
                                                labelBrush,
                                                this._xAxis.Labels.Font);
                        }
                    }

                    step = this._xAxis.Scale.MinorUnit;

                    if (step > 0 && max > min)
                    {
                        // loop to draw the minor tick marks (if needed)
                        if (this._xAxis.MinorTick != ChartTickMarkTypes.None)
                        {
                            for (int tick = min; tick <= max; tick += step)
                            {
                                // calculate tick's position on the axis
        ...
        protected virtual void OnDrawChartYAxisTitle(DrawChartEventArgs e)
        {
            ...
            // check if there is text to draw
            if (e.Cancel == false && this._yAxis.Title.Text.Length != 0)
            {
                // initialize graphic resources
                Brush textBrush = new SolidBrush(this._yAxis.Title.Color);
                try
                {
                    // draw the value axis title
                    StringFormat format = CustomGraphics.StringFormat(ContentAlignment.MiddleCenter);
                    format.FormatFlags |= StringFormatFlags.DirectionVertical;
                    e.Graphics.DrawString(this._yAxis.Title.Text, this._yAxis.Title.Font, textBrush, e.Bounds, format);
                    format.FormatFlags &= ~StringFormatFlags.DirectionVertical;
                }
                finally
                {
        ...
        protected virtual void OnDrawChartYAxis(DrawChartEventArgs e)
        {
            ...
            // check if we need draw
            if (e.Cancel == false && this._yAxis.Line.Visible && this._yAxis.Line.Weight > 0)
            {
                // determine chart's origin points
                Point start = e.Bounds.Location;
                start.Offset(0, 0);
                ...
                // initialize graphic resources
                Pen linePen = this._yAxis.Line.ToPen();
                Brush labelBrush = new SolidBrush(this._yAxis.Labels.Color);
                try
                {
                    // draw the axis line
                    e.Graphics.DrawLine(linePen, start, end);
                    ...
                    // initialize variable for tick marks drawing
                    int min = this._yAxis.Scale.Minimum;
                    int max = this._yAxis.Scale.Maximum;
                    int length = end.Y - start.Y;
                    int step = this._yAxis.Scale.MajorUnit;
                    ...
                    if (step > 0 && max > min)
                    {
                        ...
                        for (int tick = min; tick <= max; tick += step)
                        {
                            ...
                            this.DoDrawAxisTick(e.Graphics,
                                                linePen,
                                                tickCap,
                                                this._yAxis.MajorTick,
                                                true,
                                                true,
                                                this._yAxis.Labels.Visible ? tick.ToString() : null,
                                                this._yAxis.Labels.Rotation,
                                                labelBrush,
                                                this._yAxis.Labels.Font);
                        }
                    }

                    step = this._yAxis.Scale.MinorUnit;

                    if (step > 0 && max > min)
                    {
                        // loop to draw the minor tick marks (if needed)
                        if (this._yAxis.MinorTick != ChartTickMarkTypes.None)
                        {
                            for (int tick = min; tick <= max; tick += step)
                            {
                                // calculate tick's position on the axis
        ...
        protected virtual void OnDrawChartGrid(DrawChartEventArgs e)
        {
            ...
            // check if we need draw
            if (e.Cancel == false && this._grid.Line.Visible && this._grid.Line.Weight > 0)
            {
                // determine chart's origin points
                Point start = e.Bounds.Location;
                start.Offset(0, e.Bounds.Height);
                ...
                // initialize graphic resources
                Pen linePen = this._grid.Line.ToPen();
                try
                {
                    // draw the grid lines for the time axis
                    if (this._grid.XAxisStyle != ChartGridStyles.None)
                    {
                        ...
                        if (max > min && length > 0)
                        {
                            switch (this._grid.XAxisStyle)
                            {
                                case ChartGridStyles.Major:
                                    if (this._xAxis.Scale.MajorUnit > 0)
                                    {
                                        // draw major grid lines for time axis
                                        this.DoDrawXGridLines(e.Graphics, linePen, start, end, min, max, length, this._xAxis.Scale.MajorUnit);
                                    }
                                    break;

                                case ChartGridStyles.Minor:
                                    if (this._xAxis.Scale.MinorUnit > 0)
                                    {
                                        // draw minor grid lines for time axis
                                        this.DoDrawXGridLines(e.Graphics, linePen, start, end, min, max, length, this._xAxis.Scale.MinorUnit);
                                    }
                                    break;
                            }
                        }
                    ...
                    if (this._grid.YAxisStyle != ChartGridStyles.None)
                    {
                        // init scale variables
                        int min = this._yAxis.Scale.Minimum;
                        int max = this._yAxis.Scale.Maximum;
                        ...
                        if (max > min && length > 0)
                        {
                            switch (this._grid.YAxisStyle)
                            {
                                case ChartGridStyles.Major:
                                    if (this._yAxis.Scale.MajorUnit > 0)
                                    {
                                        // draw major grid lines for value axis
                                        this.DoDrawYGridLines(e.Graphics, linePen, start, end, min, max, length, this._yAxis.Scale.MajorUnit);
                                    }
                                    break;

                                case ChartGridStyles.Minor:
                                    if (this._yAxis.Scale.MinorUnit > 0)
                                    {
                                        // draw minor grid lines for value axis
                                        this.DoDrawYGridLines(e.Graphics, linePen, start, end, min, max, length, this._yAxis.Scale.MinorUnit);
                                    }
                                    break;
                            }
                        }
