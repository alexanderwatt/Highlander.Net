    ...
    public class BasicTests
    {
        ...
        public void TestIsolatedAppScopes()
        {
            using (Reference<ILogger> loggerRef = Reference<ILogger>.Create(new TraceLogger(true)))
            {
                ...
                const string cTestAppScopeA = "QDS.AU.Syd.AppA";
                const string cTestAppScopeB = "QDS.AU.Syd.AppB";
                string env = "UTT";
        ...
        public void TestConnectionProtocols()
        {
            using (Reference<ILogger> loggerRef = Reference<ILogger>.Create(new TraceLogger(true)))
            {
                ...
                using (CoreServer server = new CoreServer(loggerRef, serverSettings))
                {
                    ...
                    Assert.AreEqual(WcfConst.NetMsmq + "://" + hostIpv4 + "/private/QDS_Core_8113_ITransferV341", serverAddresses[0]);
                    Assert.AreEqual(WcfConst.NetTcp + "://" + hostIpv4 + ":8113/QDS_Core_ITransferV341", serverAddresses[1]);
                    Assert.AreEqual(WcfConst.NetPipe + "://" + hostIpv4 + "/QDS_Core_8113_ITransferV341", serverAddresses[2]);
                    // check that a client can connect using different protocols
                    string[] protocols = new string[] { WcfConst.NetTcp, WcfConst.NetMsmq, WcfConst.NetPipe };
