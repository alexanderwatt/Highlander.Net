...
using FpML.V5r10.Confirmation;
using Metadata.Common;

namespace FpML.V5r10.TestGui
{
    public partial class Form1 : Form, ITestLog
    {
        ...
        private void Log(string text)
        {
            ...
            txtLog.AppendText(String.Format("{0},{1}{2}", dt.ToString("hh:mm"), text, Environment.NewLine));
        }

        private void LogInfo(string text) { Log(String.Format("Info.: {0}", text)); }
        public void LogInfo(string text, string testId) { Log(String.Format("Info.: [{0}] {1}", testId, text)); }

        public void LogError(string text, string testId, bool fatal)
        {
            string severity = fatal ? "Fatal" : "Error";
            Log(testId != null
                    ? String.Format("{0}: [{1}] {2}", severity, testId, text)
                    : String.Format("{0}: {1}", severity, text));
            if (fatal)
                _Abort = true;
        }
