...
{
  public class CalcPLDeltaAssetPercent : ICalculation
  {
    public void Calculate(PricingResult result)
    {
      string asset = result.Find("", "Asset", nab.QR.Xml.ValueUnit.Absolute).Value.As<string>();
      result.Add(new PricingValue { Name = "PLDelta", Asset = asset, Value = new PropertyValue(CalculateValue(result)), Unit = ValueUnit.Percent });
    }
