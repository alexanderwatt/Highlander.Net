...
using Extreme.Mathematics.EquationSolvers; // root bracket classes
using Extreme.Mathematics.LinearAlgebra; // Vectors class
using Extreme.Mathematics.Optimization; // optimization classes
...
namespace Orion.Analytics.PricingEngines
{
    ...
    /// </summary>
    public class SABRCalibrationEngine : IPricingEngine
    {
        #region Constants
        ...
        /// </summary>
        private const int SequenceLength = 10000;
        ...
        public decimal ATMSlope
        {
            get { return _atmSlope; }
        }
        ...
        public decimal CalibrationError
        {
        ...
        /// 	<c>false</c>.
        /// </value>
        public bool IsATMCalibrationPossible
        {
            get { return _isATMCalibrationPossible; }
        ...
        /// 	<c>false</c>.
        /// </value>
        public bool IsFullCalibrationPossible
        {
            get { return _isFullCalibrationPossible; }
        }
        ...
        /// </value>
        public bool IsSABRModelCalibrated
        {
            get { return _isSABRModelCalibrated; }
        }
        ...
        public SABRParameters GetSABRParameters
        {
            get { return _sabrParameters; }
        }
        ...
        public decimal[,] SABRNuSurface
        {
            get { return _sabrNuSurface; }
        }
        ...
        public decimal[,] SABRRhoSurface
        {
            get { return _sabrRhoSurface; }
        }
        ...
        public decimal ThetaGuess
        {
            get { return _thetaGuess; }
        }
            ...
            (IEnumerable<KeyValuePair<SABRKey, SABRCalibrationEngine>> engineHandles)
        {
            ...
            if (engineHandles != null) BuildExpiryTenorLabels(engineHandles);
        ...
        private void FillSABRNuAndRhoSurfaces()
        {
            ...
            for (var columnIndex = 0; columnIndex < numTenors; ++columnIndex)
            {
                ...
                var nuInterpObj =
                    new LinearInterpolation(ref xValues, ref nuValues);
                var rhoInterpObj =
                    new LinearInterpolation(ref xValues, ref rhoValues);

                foreach (var target in targets)
                {
                    var nu = nuInterpObj.Interpolate(target);
                    var rho = rhoInterpObj.Interpolate(target);
        ...
        {
            var initalGuess =
                new GeneralVector(decimal.ToDouble(_thetaGuess),
                                  decimal.ToDouble(_muGuess));
            // Calibrate SABR model.
            MultivariateRealFunction objectiveFunction = ObjectiveFunction;
            var optimizer = new NelderMeadOptimizer
                                {
                                    InitialGuess = initalGuess,
            ...
            _isSABRModelCalibrated =
                (optimizer.Status == AlgorithmStatus.Converged);
            _calibrationError = (decimal) optimizer.ValueAtExtremum;
        }
        ...
        {
            // Set the inputs required by the equation solver.
            RealFunction equation = TargetFunction;
            ...
            RootBracketingSolver solver = new DekkerBrentSolver
                                              {
                                                  LowerBound =
                                                      decimal.ToDouble(Math.Min(MinimumAlpha, alpha/AlphaMultiplier)),
                                                  UpperBound = decimal.ToDouble(upperBound),
                                                  TargetFunction = equation
                                              };
            // Solve for SABR parameter alpha and store result.
            try
            {
                _sabrParameters.Alpha = (decimal)solver.Solve();
            }
            catch (Exception)
            {
        ...
        /// compatible with the MultivariateRealFunction delegate in 
        /// the Extreme Optimization library.
        /// </summary>
        ...
        /// used the Extreme Optimization library. 
        /// </summary>
        ...
        /// <summary>
        /// Name that identifies the SABR calibration engine object.
        /// </summary>
