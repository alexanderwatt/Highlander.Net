...
namespace Orion.Analytics.Options
{
    ...
    public class BlackScholesMertonModel
    {
        ...
        /// <param name="t">Time in years to the maturity of the option.</param>
        /// <returns>An array of reuslts for Black Scholes.</returns>
        private void Calculate(Boolean callFlag, double fwdPrice, double strike, double vol, double t)//TODO put back rate.
        {
            if (fwdPrice < 0) throw new ArgumentOutOfRangeException("fwdPrice", "Invalid parameter in option evaluation");
            if (vol < 0) throw new ArgumentOutOfRangeException("vol", "Invalid parameter in option evaluation");
            if (t < 0) throw new ArgumentOutOfRangeException("t", "Invalid parameter in option evaluation");

            string key = string.Format("{0},{1},{2},{3}", fwdPrice, strike, vol, t);
            ...
            else if (strike <= 0)
            {
                if (!callFlag)
                {
                    throw new ArgumentOutOfRangeException("callFlag", "If strike is less than 0 then option must be a put.");
                }
