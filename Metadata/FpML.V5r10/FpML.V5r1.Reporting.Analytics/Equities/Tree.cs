...
namespace Orion.Analytics.Equities 
{
  ...
  public class Tree
  {
    ...
    //get SpotMatrix item
    public double Get_SpotMatrix(int idx, int jdx)
    {
      return SpotMatrix[idx, jdx];
    }

    //set SpotMatrix item
    public void Set_SpotMatrix(int idx, int jdx, double value)
    {
    ...
    public void MakeGrid(ZeroCurve myZero, DivList myDivList)
    {
      ...
      //make the spot grid
      double sv = Spotstar;
      Set_SpotMatrix(0, 0, sv+get_div(0)) ;
      ...
      if (myZero != null)
      {
        ...
        for (int idx = 0; idx < Gridsteps; idx++)
        {
          if (flatFlag)
            ...
            set_r(idx, myZero.forwardRate(idx * dt, (idx + 1) * dt));
        }
      }
    }
    ...
    private void FillUpDown(double sig)
    {
      if (up != null)
      {
        ...
        for (int idx = 0; idx < Gridsteps; idx++)
        {
          double a = (1.0 + Math.Exp((2 * get_r(idx) + sig * sig) * dt));
          double upval = (a + Math.Sqrt(a * a - 4.0 * Math.Exp(2.0 * get_r(idx) * dt)))
              / (2.0 * Math.Exp(get_r(idx) * dt));
          set_up(idx, upval);
