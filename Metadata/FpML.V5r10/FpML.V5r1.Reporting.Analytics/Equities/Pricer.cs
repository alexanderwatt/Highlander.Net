...
namespace Orion.Analytics.Equities 
{
  public class Pricer
  {
    ...
    private void FillP(Tree SpotTree)
    {
      ...
      {
        temp = (Math.Exp(SpotTree.get_r(i) * dt) - SpotTree.get_dn(i)) /
    ...
    private void TermValue(Tree SpotTree)
    {
      if (SpotTree != null)
      {
        for (int j = 0; j <= Gridsteps; j++)
        {
          ...
          {
            temp = Math.Max(SpotTree.Get_SpotMatrix(Gridsteps, j) - Strike, 0);
            Set_PriceMatrix(Gridsteps, j, temp);
          }
          else if ((Payoff.ToLower() == "p") | (Payoff.ToLower() == "put"))
          {
            temp = Math.Max(Strike - SpotTree.Get_SpotMatrix(Gridsteps, j), 0);
            Set_PriceMatrix(Gridsteps, j, temp);
          ...
          for (int j = 0; j <= Gridsteps - 1; j++)
              ...
              if ((Style.ToLower() == "a") | (Style.ToLower() == "american"))
                  ...
                  if ((Payoff.ToLower() == "c") | (Payoff.ToLower() == "call"))
                      {
                      temp = Math.Max(Math.Exp(-SpotTree.get_r(idx) * dt) * (Get_P(idx) *
                          Get_PriceMatrix(idx+1, j + 1) + (1 - Get_P(idx)) *
                          Get_PriceMatrix(idx+1, j)), SpotTree.Get_SpotMatrix(idx, j)
                          - Strike);
                  ...
                  {
                  temp = Math.Exp(-SpotTree.get_r(idx) * dt) * (Get_P(idx) *
                         Get_PriceMatrix(idx+1, j + 1) + (1 - Get_P(idx )) *
      ...
      private void Smooth(string str, Tree SpotTree)
      {
        if (str != null)
        {
          ...
          if ((str.ToLower() == "y") | (str.ToLower() == "yes"))
          {
            ...
            for (k = 1; ((SpotTree.Get_SpotMatrix(idx, k - 1) <= Strike) &&
                        ((SpotTree.Get_SpotMatrix(idx, k) <= Strike)) &&
                        ((k <= Gridsteps - 1))); k++)
            {
            }
            if (k == 1)
            ...
            {
              if (Math.Abs(SpotTree.Get_SpotMatrix(idx, k - 2) / Strike - 1) >
                  Math.Abs(SpotTree.Get_SpotMatrix(idx, k + 1) / Strike - 1))
              {
                Centre = k;
              }
              else
            ...
            for (j = Centre - 1; j <= Centre + 1; j++)
            {
              ...
              if ((Payoff.ToLower() == "c") || (Payoff.ToLower() == "call"))
              {
                ...
                double fwd = SpotTree.Get_SpotMatrix(idx, j) - SpotTree.get_div(idx);
                fwd *= Math.Exp(SpotTree.get_r(idx) * dt);
                temp = BSprice(fwd, dt, Strike, SpotTree.get_r(idx), SpotTree.Sig, "C");
                Set_PriceMatrix(idx, j, temp);
              }
              else if ((Payoff.ToLower() == "p") || (Payoff.ToLower() == "put"))
              {
                ...
                double fwd = SpotTree.Get_SpotMatrix(idx, j) - SpotTree.get_div(idx);
                fwd *= Math.Exp(SpotTree.get_r(idx) * dt);
                temp = BSprice(fwd, dt, Strike, SpotTree.get_r(idx), SpotTree.Sig, "P");
                Set_PriceMatrix(idx, j, temp);
              }
            }
    ...
    public void MakeGrid(Tree SpotTree)
    {
      ...
      if ((Style.ToLower() == "a") | (Style.ToLower() == "american"))
        for (int i = Gridsteps - 2; i >= 0; i--)
        {
          for (int j = 0; j <= i; j++)
          {
            if ((Payoff.ToLower() == "c") | (Payoff.ToLower() == "call"))
            {
              temp = Math.Max(Math.Exp(-SpotTree.get_r(i) * dt) * (Get_P(i) *
                  Get_PriceMatrix(i + 1, j + 1) + (1 - Get_P(i)) *
                  Get_PriceMatrix(i + 1, j)), SpotTree.Get_SpotMatrix(i, j)
                  - Strike);
            ...
            else if ((Payoff.ToLower() == "p") | (Payoff.ToLower() == "put"))
            {
              temp = Math.Max(Math.Exp(-SpotTree.get_r(i) * dt) * (Get_P(i) *
                 Get_PriceMatrix(i + 1, j + 1) + (1 - Get_P(i)) *
                 Get_PriceMatrix(i + 1, j)), Strike - SpotTree.Get_SpotMatrix(i, j));
              Set_PriceMatrix(i, j, temp);
            }
          }
        }
      else if ((Style.ToLower() == "e") | (Style.ToLower() == "european"))
      {
        for (int i = Gridsteps - 2; i >= 0; i--)
        {
          for (int j = 0; j <= i ; j++)
          {
            temp = Math.Exp(-SpotTree.get_r(i) * dt) * (Get_P(i) *
                   Get_PriceMatrix(i + 1, j + 1) + (1 - Get_P(i)) *
