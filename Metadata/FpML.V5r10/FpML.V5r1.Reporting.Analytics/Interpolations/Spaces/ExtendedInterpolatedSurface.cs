...
namespace Orion.Analytics.Interpolations.Spaces
{
    ...
    public class ExtendedInterpolatedSurface : InterpolatedSurface
    {
        ...
        ///<summary>
        /// The forward must be set or the appropriate interpoalted expiry.
        ///</summary>
        public double? Forward { get; set; }

        ///<summary>
        /// The spot muct be set for each Value call.
        ///</summary>
        ...
        public int Columns
        {
            get
            {
                return _interpolatedColumns.Count;
            }
        }
            ...
            if (yInterpolation.GetType() == typeof(SABRModelInterpolation))
            {
                ...
                for (int i = 0; i < length; i++)
                {
                    ...
                    if (Forward != null && Spot != null)                    
                        yinterp.AssetPrice = System.Convert.ToDecimal(Forward);                    
                    else
                    {
                        var fwd = ForwardCurve.ValueAt(yinterp.ExpiryTime, true);
                        yinterp.AssetPrice = System.Convert.ToDecimal(fwd);
                    }
        ...
        /// </summary>
        /// <param name="columns">The column values.</param>
        /// <param name="forwards">An array of forwards to the expiry dates. The length of this arrat is + 1, as the first elenment is the spot value.</param>
        /// <param name="values">The discrete surface ie 2 dimensional.</param>
        /// <param name="xInterpolation">The basic interpolation to be applied to the x axis.</param>
        /// <param name="yInterpolation">The basic interpolation to be applied to the y axis.</param>
        /// <param name="rows">The row values.</param>
        ...
        {
            double result=0.0;
            if (_yInterpolation.GetType() == typeof(SABRModelInterpolation))
            ...
            else
            {
                ...
                if (_yInterpolation.GetType() == typeof(WingModelInterpolation))
                {
                    if (Forward != null && Spot != null)
                    {
                        ((WingModelInterpolation)_yInterpolation).Forward = (double)Forward;
                        ((WingModelInterpolation)_yInterpolation).Spot = (double)Spot;
                    }
                    else
                    {
                        var fwd = ForwardCurve.ValueAt(x, true);
                        ((WingModelInterpolation)_yInterpolation).Forward = fwd;
                        ((WingModelInterpolation)_yInterpolation).Spot = SpotValue;
                    }
                    var discreteCurve = new DiscreteCurve(surface.YArray, xArray);
                    InterpolatedCurve = new InterpolatedCurve(discreteCurve, _yInterpolation, true);
        ...
        private double SABRValue(IPoint point)
        {
            ...
            if (yList.Count == 1)
                interp.AtmVolatility = System.Convert.ToDecimal(vList[0]);
            else
            {
                var xDiscCurve = new DiscreteCurve(yList.ToArray(), vList.ToArray());
                var xInterp = new InterpolatedCurve(xDiscCurve, _xInterpolation, true);
                IPoint atmpt = new Point1D(x);
                double temp = xInterp.Value(atmpt);
                interp.AtmVolatility = System.Convert.ToDecimal(temp);
            }
            if (Forward != null && Spot != null)
                interp.AssetPrice = System.Convert.ToDecimal(Forward);
            else
            {
                var fwd = ForwardCurve.ValueAt(interp.ExpiryTime, true);
                interp.AssetPrice = System.Convert.ToDecimal(fwd);
            }
