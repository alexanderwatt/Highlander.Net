...
namespace FpML.V5r10.Reporting.Analytics.Rates
{
    ...
    public static class RateAnalytics
    {
                                                      ...
                                                      double compoundingPeriod)
        {
            ...
            {
                result = -System.Math.Log(discountFactor) / yearFraction;
            }
            else
            {
                double power = -compoundingPeriod / yearFraction;
                result = (System.Math.Pow(discountFactor, power) - 1.0) / compoundingPeriod;
            }
            return result;
        }
            ...
            CompoundingFrequencyEnum compoundingFrequency)
        {
            ...
            if (compoundingPeriod == 0)
            {
                result = (decimal)System.Math.Exp(Convert.ToDouble(-rate * yearFraction));
            }
            else
            {
                decimal df;
                ...
                {
                    df = (decimal)System.Math.Pow(1 + compoundingPeriod * (double)rate, (double)yearFraction);
                }
                else
                {
