...
namespace Orion.Analytics.Rates
{
    public static class OsirisAnalytics
    {
        ...
        public static Double GetAverageVolatility(Double T, Double R, Double Sigma)
        {
            ...
            Double M1 = (Math.Exp(R * T) - 1) / (R * T);     // First Moment
            Double M2 = 2 * Math.Exp((2 * R + SigmaSq) * T) / ((R + SigmaSq) * (2 * R + SigmaSq) * T * T) +
                            2 * (1 / (2 * R + SigmaSq) - Math.Exp(R * T) / (R + SigmaSq)) / (R * T * T);
            SigmaSq = Math.Log(M2 / (M1 * M1)) / T;  // = [ln(M2) - 2*ln(M1)]/T = ln(M2)/T - 2(R - qA)
            Double SigmaAvg = Math.Sqrt(SigmaSq);
            return SigmaAvg;
        }
        ...
        /// Payment_Rateset is the sum of the rates that have already been fixed in the period.
        ...
        /// Hoidays:	Index Start is appropriate working day for the Average Date
        ...
        /// <param name="indexCurveCode"></param>
        ...
        /// <returns></returns>
        public static Double GetAverageRate(DateTime valueDate, IRateCurve discountCurve, int indexCurveCode, IRateCurve indexCurve,
                                              DateTime paymentStart, DateTime paymentEnd, int indexDaycount, int indexTenor, int indexCoupon,
                                              double paymentRateset, double indexRateset, int averagingFrequency,
                                              int averagePreset, long indexCity, int convention, double volatilityMargin, IVolatilitySurface indexVolatility)
        {
            long indexSet, indexStart, indexEnd;
            double SumRate = 0.0, NewRate = 0.0, FirstRate, LastRate;
            double SumVol = 0.0, NewVol = 0.0, FirstVol = 0.0, LastVol = 0.0;
            var averageDate = paymentStart;
            int numAvePoints = 0;
            if (indexCurve is null)
                return 0;

            CString SWIFTCode = GetSWIFT_String(IndexCurveCode);
            ...
            if (AveFrequency <= 0) 
            {
                ...
                Date Ave_Index_Start, Ave_Index_End;
                long First_Set, Last_Set;
                //Calculate the fixing date for the start date of the period
                First_Set = AddWorkDays(PaymentStart, Ave_Preset, Index_City, CHoliday::PRECEEDING);
                //Calculate the fixing date for the end date of the period
                Last_Set = AddWorkDays(PaymentEnd, Ave_Preset, Index_City, CHoliday::PRECEEDING);

                if (Value_Date >= Last_Set)
                {
                    IndexVolatility = 0.0;
                ...
                if (Value_Date < First_Set)
                {
                    ...
                    long IndexExpiry = (long)(First_Set - Value_Date);

                    FirstRate = _GetIndex(Value_Date, First_Set, Index_Start, Index_End, Index_Tenor,
                    ...
                    FirstVol = GetVolatility(CapsVolName, SwaptVolName, IndexExpiry, Index_Tenor_Days, 0.0) + VolatilityMargin;
                    // SAJ 25/9/96	Added following lines for convexity effect
                    double Weight, AdjForward;
                    GetModifiedForward(Value_Date, PaymentEnd, First_Set, Index_Start, Index_End,
                ...
                Index_End = GoodDay(Ave_Index_End.GetJulian(), Index_City, Convention);
                long Index_Tenor_Days = (long)(Index_End - Index_Start);
                long IndexExpiry = (long)(Last_Set - Value_Date);
                LastRate = _GetIndex(Value_Date, Last_Set, Index_Start, Index_End, Index_Tenor,
                                      Index_Daycount, Index_Coupon, Index_Coupon, *pIndexCurve, Index_Rateset);
                // SAJ 25/9/96  Added Volatility margin to volatility to allow for bumping
                LastVol = GetVolatility(CapsVolName, SwaptVolName, IndexExpiry, Index_Tenor_Days, 0.0) + VolatilityMargin;
                // SAJ 25/9/96	Added following lines for convexity effect
                double Weight, AdjForward;
                GetModifiedForward(Value_Date, PaymentEnd, Last_Set, Index_Start, Index_End,
                                                Index_Tenor, Index_Coupon, Index_Daycount,
            ...
            while (AverageDate < PaymentEnd)
            {
                ...
                if (Value_Date < Index_Set)
                {
                    ...
                    NewRate = _GetIndex(Value_Date, Index_Set, AverageDate, Index_End, Index_Tenor,
                                      Index_Daycount, Index_Coupon, Index_Coupon, *pIndexCurve, Index_Rateset);

                    // SAJ 25/9/96  Added Volatility margin to volatility to allow for bumping
                    NewVol = GetVolatility(CapsVolName, SwaptVolName, IndexExpiry, Index_Tenor_Days, 0.0) + VolatilityMargin;
                ...
                AverageDate = GoodDay(AverageDate, Index_City, CHoliday::FOLLOWING);  // Use following to prevent chattering
                                                                                      // SAJ 25/9/96 End
            }
            IndexVolatility = sqrt(SumVol / NumAvePoints) + VolatilityMargin;
        ...
        /// <returns></returns>
        public static Double ExchangeCapValue(double price1, double price2, double strike, double time, double volatility1, double volatility2, double corr)
        {
            var volatility = 0.0;
        ...
        /// <param name="corr"></param>
        /// <returns></returns>
        public static Double ExchangeCapTheta(double price1, double price2, double strike, double time, double volatility1, double volatility2, double corr)
        {
            var volatility = 0.0;
            double temp = volatility1 * volatility1 + volatility2 * volatility2 - 2 * corr * volatility1 * volatility2;
            if (Math.Abs(temp) > 0)
        ...
        /// <param name="corr"></param>
        /// <returns></returns>
        public static Double ExchangeCapDelta(double price1, double price2, double strike, double time, double volatility1, double volatility2, double corr)
        {
            var volatility = 0.0;
            double temp = volatility1 * volatility1 + volatility2 * volatility2 - 2 * corr * volatility1 * volatility2;
            if (Math.Abs(temp) > 0)
        ...
        DllExport double DECORATE Exchange_Cap_Gamma(double price1, double price2, double strike, double time, double volatility1, double volatility2, double corr)

        {
            double Cap = 0.0, volatility = 0.0;

            volatility = sqrt(volatility1 * volatility1 + volatility2 * volatility2 - 2 * corr * volatility1 * volatility2);
        ...
        DllExport double DECORATE Exchange_Cap_Vega(double price1, double price2, double strike, double time, double volatility1, double volatility2, double corr)
        ...
        DllExport double DECORATE Exchange_Floor_Value(double price1, double price2, double strike, double time, double volatility1, double volatility2, double corr)
        ...
        DllExport double DECORATE Exchange_Floor_Theta(double price1, double price2, double strike, double time, double volatility1, double volatility2, double corr)
        ...
        DllExport double DECORATE Exchange_Floor_Delta(double price1, double price2, double strike, double time, double volatility1, double volatility2, double corr)
        ...
        DllExport double DECORATE Exchange_Floor_Gamma(double price1, double price2, double strike, double time, double volatility1, double volatility2, double corr)
        ...
        DllExport double DECORATE Exchange_Floor_Vega(double price1, double price2, double strike, double time, double volatility1, double volatility2, double corr)
        ...
        DllExport double DECORATE Exchange_Digital_Cap_Value(double payout, double price1, double price2, double strike, double time, double volatility1, double volatility2, double corr)
        ...
        DllExport double DECORATE Exchange_Digital_Cap_Theta(double payout, double price1, double price2, double strike, double time, double volatility1, double volatility2, double corr)
        ...
        DllExport double DECORATE Exchange_Digital_Cap_Delta(double payout, double price1, double price2, double strike, double time, double volatility1, double volatility2, double corr)
        ...
        DllExport double DECORATE Exchange_Digital_Cap_Gamma(double payout, double price1, double price2, double strike, double time, double volatility1, double volatility2, double corr)
        ...
        DllExport double DECORATE Exchange_Digital_Cap_Vega(double payout, double price1, double price2, double strike, double time, double volatility1, double volatility2, double corr)
        ...
        DllExport double DECORATE Exchange_Digital_Floor_Value(double payout, double price1, double price2, double strike, double time, double volatility1, double volatility2, double corr)
        ...
        DllExport double DECORATE Exchange_Digital_Floor_Theta(double payout, double price1, double price2, double strike, double time, double volatility1, double volatility2, double corr)
        ...
        DllExport double DECORATE Exchange_Digital_Floor_Delta(double payout, double price1, double price2, double strike, double time, double volatility1, double volatility2, double corr)
        ...
        DllExport double DECORATE Exchange_Digital_Floor_Gamma(double payout, double price1, double price2, double strike, double time, double volatility1, double volatility2, double corr)
        ...
        DllExport double DECORATE Exchange_Digital_Floor_Vega(double payout, double price1, double price2, double strike, double time, double volatility1, double volatility2, double corr)
        ...
        DllExport double DECORATE XGetIndex(int cType, long ValueDate, long aSetdate, long aStartDate, long aMaturityDate,
...
{ 
   ...
   {
		case 1:	// Libor-FRA
			return GetFRAFloat(ValueDate, aStartDate, aMaturityDate, aFrequency, aRateSet, aRollType, aAccessCityCode, aGenMethod, DayCount,
                                aInitialprice, aFinalprice, Indextype, DisCurve, IndexCurve);
			break;
		case 2:	// Libor-GenSwap
			return 0.0; // GetGenSwap( ValueDate, aStartDate, aMaturityDate, aFrequency, aRollType, aAccessCityCode, aGenMethod, DayCount,
						//		aInitialprice, aFinalprice, Indextype, Curve, DisCurve);
			break;
		case 3:	// Libor-Zero
			return 0.0; // GetZero(ValueDate, aSetdate, aStartDate, aMaturityDate, aFrequency, DayCount, aFrequency, rFrequency, Curve, aIndexset);
			break;
		case 4:	// Libor-Fixed
			return GetFixed(ValueDate, aStartDate, aMaturityDate, aFrequency, aRateSet, aRollType, aAccessCityCode, aGenMethod, DayCount,
                                aInitialprice, aFinalprice, Indextype, DisCurve);
			break;
		case 5:	// Libor-Floating
			return GetFloat(ValueDate, aStartDate, aMaturityDate, aFrequency, aRateSet, aRollType, aAccessCityCode, aGenMethod, DayCount,
                                aInitialprice, aFinalprice, Indextype, DisCurve, IndexCurve);
			break;
//		case 6:	// Bond
                                                ...
                                                double aInitialprice, double aFinalprice, int Indextype, const PVCurve &DisCurve)
{
    ...
    for (short Index = 0; Index < ArraySize; Index++)
    {
        ...
        PVArray[Index] = (double)DisCurve.GetLogLinearPoint((double)(((Date*)OurDates[Index + 1])->GetJulian()));
    ...
    {
        case 0: // Value
            IndexNum = PVFinal * aFinalprice - PVInitial * aInitialprice + (mRateSet * SwapDelta1(0.0, 0.0, PVArray, DayArray, ArraySize));
            break;
        case 1: // Index
            IndexNum = SwapRate1(PVInitial * aInitialprice, PVFinal * aFinalprice, PVArray, DayArray, ArraySize);
            break;
        case 2: // Delta0
            IndexNum = 0.0;
            break;
        case 3: //Delta1
            ...
            break;
        case 4: //Delta01
            IndexNum = (SwapDelta1(PVInitial * aInitialprice, PVFinal * aFinalprice, PVArray, DayArray, ArraySize)) * 1e-4;
            break;
        case 5: //Gamma0
            IndexNum = 0.0;
            break;
...
double GetFloat(long ValueDate, long aStartDate, long aMaturityDate, int aFrequency, double mRateSet,
                                                int aRollType, int aAccessCityCode, int aGenMethod, int DayCount,
                                                double aInitialprice, double aFinalprice, int Indextype, const PVCurve &DisCurve, const PVCurve &IndexCurve)
{
    int ArraySize;
    double* PVArray;
    double* IndexArray;
...

    {
        for (short Index = 0; Index < ArraySize; Index++)
        {
            ...
            IndexArray[Index] = (double)IndexCurve.GetLogLinearPoint((double)(((Date*)OurDates[Index + 1])->GetJulian()));
    ...
    {
        case 0: // Value
            IndexNum = PVFinal * (1.0 - aFinalprice) - PVInitial * (1.0 - aInitialprice) + (mRateSet * SwapDelta1(0.0, 0.0, IndexArray, DayArray, ArraySize));
            break;
        case 1: // Index
            IndexNum = SwapRate1(PVInitial * aInitialprice, PVFinal * aFinalprice, PVArray, DayArray, ArraySize) - SwapRate1(PVInitial * aInitialprice, PVFinal * aFinalprice, IndexArray, DayArray, ArraySize);
            break;
        case 2: // Delta0
        ...
        case 3: //Delta1
            ...
            break;
        case 4: //Delta01
            IndexNum = (SwapDelta1(0.0, 0.0, PVArray, DayArray, ArraySize) - SwapDelta1(0.0, 0.0, IndexArray, DayArray, ArraySize)) * 1e-4;
            break;
        case 5: //Gamma0
            IndexNum = 0.0;
            break;
                                                ...
                                                double aInitialprice, double aFinalprice, int Indextype, const PVCurve &DisCurve, const PVCurve &IndexCurve)
{
    ...
    {
        case 0: // Value
            IndexNum = PVFinal * (1.0 - aFinalprice) - PVInitial * (1.0 - aInitialprice) + (mRateSet * DayArray * IndexFinal);
            break;
        case 1: // Index
...
double SwapGamma1(double* PVi, double* Di, int NoOfPoints)
{
    ...
    {
        Alpha += Di[Index];
        SwapGamma += fabs(PVi[Index] * Di[Index] * Alpha);
    }
...
DllExport long DECORATE XGenerateDates(DateArray& Vector,
                                        int aGenMethod,
                                        long aStartDate,
                                        long aMaturityDate,
                                        int aFrequency,
...

{
    ...
    long ArraySize = GenerateDates(aGenMethod, aStartDate, aMaturityDate, aFrequency, aRollType, aAccessCityCode, Cfdates);

    Vector.SetSize(int(ArraySize) + 1);

    for (int loop = 0; loop < Vector.GetSize(); loop++)
    {

        Vector[loop] = new Date(Cfdates[loop]);
