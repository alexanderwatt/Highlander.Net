    ...
    public class ThreadQueueTests
    {
        ...
        public void TestQueueOrdering()
        {
            ...
            void Callback(int data)
            {
                ...
                try
                {
                    if (threads > 1)
                    {
                        ...
                        if (errors <= 50) loggerRef.Target.LogDebug("Thread count = {0}!", threads);
                    }

                    if (count != (data + 1))
                    {
                        int errors = Interlocked.Increment(ref totalErrors);
                        if (errors <= 50) loggerRef.Target.LogDebug("Recv count = {0} but (data+1) = {1}", count, data + 1);
                    }
                }
                finally
                {
            ...
            for (int i = 0; i < iterations; i++)
            {
                ...
                queue.Dispatch(i, Callback);
            }
            Assert.AreEqual(iterations, sendCount);
            long itemsRemaining = queue.WaitUntilEmpty(TimeSpan.FromSeconds(30));
            Assert.AreEqual(0, itemsRemaining);
            Assert.AreEqual(iterations, recvCount);
        ...
        public void TestQueueConcurrency()
        {
            ...
            Assert.AreEqual(0, itemsRemaining);
            Assert.AreEqual(0, totalErrors);
            Assert.AreEqual(iterations, callbacksCommenced);
            Assert.AreEqual(iterations, callbacksCompleted);
        }
        ...
        public void TestPriorityQueue()
        {
            ...
            int[] lastRecv = new int[nPriorities];
            ...
            {
                int count = Interlocked.Increment(ref recvCount);
                // check values are received in priority order
                int i = value % 1000000;
                int priority = (value - i) / 1000000;
                int last = lastRecv[priority];
                if (i <= last)
                {
                    int errors = Interlocked.Increment(ref excpCount);
                    if (errors <= 50) loggerRef.Target.LogDebug("Priority={0}: i ({1}) not greater than previous ({2})!", priority, i, last);
                }
            ...
            for (int i = 0; i < iterations; i++)
            {
                ...
                queue.Dispatch(value, Callback, priority);
            }
            Assert.AreEqual(iterations, sendCount);
            long itemsRemaining = queue.WaitUntilEmpty(TimeSpan.FromSeconds(30));
            Assert.AreEqual(0, itemsRemaining);
        ...
        public void TestPriorityStack()
        {
            ...
            for (int i = 0; i < iterations; i++)
            {
                ...
                AsyncQueuePriority priority = (AsyncQueuePriority)(i % nPriorities);
                int value = (int)priority * 1000000 + i;
                stack.Dispatch(value, Callback, priority, (i % 1000).ToString());
            }
            Assert.AreEqual(iterations, sendCount);
