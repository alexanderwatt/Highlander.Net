...
namespace Highlander.Utilities.Encryption
{
    internal class KeyTriple
    {
        ...
        public KeyTriple(string id, int alg, string transportKey, string publicKey, string secretKey)
        {
    ...
    public interface ICryptoManager
    {
        ...
        string GetTransportKey(string id);
        void SetTransportKey(string id, string transportKey);
        string GetPublicKey(string id);
        void SetPublicKey(string id, string publicKey);
        byte[] DecryptWithTransportKey(string id, byte[] buffer);
        byte[] EncryptWithTransportKey(string id, byte[] buffer);
    ...
    public class DefaultCryptoManager : ICryptoManager
    {
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public string GetTransportKey(string id)
        {
            if (id == null)
        ...
        public void SetTransportKey(string id, string transpKey)
        {
            if (id == null)
                throw new ArgumentNullException(nameof(id));
            if (!_keyCache.TryGetValue(id, out var keys))
        ...
        /// <returns></returns>
        public byte[] EncryptWithTransportKey(string id, byte[] buffer)
        {
            if (buffer == null)
                return null;
            if (!_keyCache.TryGetValue(id, out var keys))
                throw new ArgumentException("Invalid key id", nameof(id));
            if (keys.TransportKey == null)
                throw new ArgumentException("Transport key not set for key id: '" + id + "'");
            switch (keys.Alg)
            {
                case 1: // Phelix
                    PhelixSession phx = new PhelixSession(keys.TransportKey);
        ...
        /// <param name="buffer"></param>
        /// <returns></returns>
        public byte[] DecryptWithTransportKey(string id, byte[] buffer)
        {
            if (buffer == null)
                return null;
            if (!_keyCache.TryGetValue(id, out var keys))
    ...
    public class PhelixSession
    {
        ...
        public PhelixSession(string sessionKey)
        {
            ...
            mac = new byte[Phelix.PhelixMacSize96 / 8];
        }
        ...
        public byte[] EncryptBuffer(byte[] plainBuffer)
        {
            ...
            var nonce = new byte[Phelix.PhelixNonceSize / 8];
            rng.GetBytes(nonce);
            ms.Write(nonce, 0, nonce.Length);
            // set up the cipher
            phelix.Init();
            phelix.SetupKey(key, 0, key.Length * 8, mac.Length * 8);
            phelix.SetupNonce(nonce, 0);
            var ctxt = new byte[plainBuffer.Length];
            // encrypt and store the whole string
            phelix.EncryptBytes(plainBuffer, 0, ctxt, 0, ctxt.Length);
            ms.Write(ctxt, 0, ctxt.Length);
            // calculate the 128bit MAC and append it
            phelix.Finalize(mac, 0);
            ms.Write(mac, 0, mac.Length);
            byte[] returnBuffer = ms.ToArray();
            return returnBuffer;
        }

        const int OVERHEAD_SIZE_BYTES = (Phelix.PhelixNonceSize + Phelix.PhelixMacSize96) / 8;
        ...
        public byte[] DecryptBuffer(byte[] cipherBuffer)
        {
            ...
            // set up the cipher
            phelix.Init();
            phelix.SetupKey(key, 0, this.key.Length * 8, mac.Length * 8);
            phelix.SetupNonce(cipherBuffer, 0);
            // decrypt and decode the string
            var ptxt = new byte[cipherBuffer.Length - OVERHEAD_SIZE_BYTES];
            phelix.DecryptBytes(cipherBuffer, Phelix.PhelixNonceSize / 8, ptxt, 0, ptxt.Length);
            // check the MAC
            phelix.Finalize(mac, 0);
            var nOfs = Phelix.PhelixNonceSize / 8 + ptxt.Length;
