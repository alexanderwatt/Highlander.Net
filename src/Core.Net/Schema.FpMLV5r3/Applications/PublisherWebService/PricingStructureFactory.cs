    ...
    internal static class PricingStructureFactory
    {
        ...
        internal static IPricingStructure CreatePricingStructure(ILogger logger, ICoreCache cache, string nameSpace, object[][] propertiesRange, object[][] values, object[][] additional)
        {
            ...
            for (int row = 0; row <= rowCount; row++)
            {
                ...
                volatility.Add((double)values[row][1]);
            }
            ...
            return new RateVolatilitySurface(logger, cache, nameSpace, properties, expiries.ToArray(), volatility.ToArray(), inputInstruments.ToArray(),
        ...
        internal static IPricingStructure CreatePricingStructure(ILogger logger, ICoreCache cache, string nameSpace, object[][] propertiesRange, object[][] values)
        {
            ...
            {
                return CurveEngine.V5r3.Factory.PricingStructureFactory.CreatePricingStructure(logger, cache, nameSpace, null, null, properties, ConvertObjectArray(values));
            }
            var expiries = ExtractExpiries(values);
            var volatilities = ExtractVolatility(values, expiries.Length);
            ...
            if (PricingStructureHelper.VolSurfaceTypes.Contains(psType))
            {
                ...
                return CurveEngine.V5r3.Factory.PricingStructureFactory.CreateVolatilitySurface(logger, cache, nameSpace, properties, expiries, strikes, volatilities);
            }
        ...
        /// <summary>
        /// Extract the volatility from the raw Data grid (everything except the first column and row)
        /// </summary>
        private static double[,] ExtractVolatility(object[][] rawGrid, int maxRows)
        {
