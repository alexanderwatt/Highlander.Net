...
namespace Highlander.Models.Tests.V5r3.Rate
{
    ...
    public class CouponAnalyticTests
    {
        ...
        public void TestBucketedDelta1()
        {
            ...
            IRateCouponParameters[] analyticModelParameters = new IRateCouponParameters[len - 1];
            int index = 0;
            for (int i = 1; i < len; ++i)
            {
                IRateCouponParameters cp = new RateCouponParameters();
                cp.BucketedDates = CreateArray(_bucketedDates, 0, i + 1);
                cp.BucketedDiscountFactors = CreateArray(_bucketedDiscountFactors, 0, i + 1);
                analyticModelParameters[index] = cp;
                FixedRateCouponAnalytic am = new FixedRateCouponAnalytic {AnalyticParameters = cp};
        ...
        public void Test2BucketedDelta1()
        {
            ...
            IRateCouponParameters[] analyticModelParameters = new IRateCouponParameters[ len ];
