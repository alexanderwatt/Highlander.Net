...
namespace Highlander.CurveEngine.V5r3.Assets.Rates.Futures
{
    ///<summary>
    ///</summary>
    public class PriceableRateFuturesAsset : PriceableRateSpreadAssetController, IPriceableClearedRateAssetController, IPriceableFuturesAssetController
    {
                                         ...
                                         BasicQuotation marketQuote, decimal extraQuote)
        {
            ...
            var expiryDate = LastTradingDayHelper.GetLastTradingDay(baseDate, exchangeCommodityName, Code);
            var period = FuturesLag.Negative();
            var backwardLag = RelativeDateOffsetHelper.Create(period, FuturesLag.dayType, FuturesLag.businessDayConvention, FuturesLag.businessCenters);
            LastTradeDate = ((BaseCalendar)rollCalendar).Advance(expiryDate, backwardLag, convention);
            ...
            {
                AdjustedStartDate = ((BaseCalendar)rollCalendar).Advance(expiryDate, FuturesLag, convention);
            ...
            else
            {
                ...
                AdjustedStartDate = ((BaseCalendar)rollCalendar).Advance(expiryDate, offset, BusinessDayConventionEnum.NONE);
