...
namespace Highlander.Core.Server
{
    internal partial class PackageQueryBase
    {
        ...
        public PackageQueryBase(V341SelectMultipleItems request)
        {
            ...
            ItemKind = request.QueryDef.ItemKind.ToItemKind();
            ItemIds = request.ItemIds.Select(i => Guid.Parse(i)).ToArray();
            ItemNames = request.QueryDef.ItemNames.ToArray();
            QueryExpr = request.QueryDef.QueryExpr;
            AppScopes = request.QueryDef.AppScopes.ToArray();
            MinimumUSN = request.QueryDef.MinimumUSN;
            ExcludeDeleted = request.QueryDef.ExcludeDeleted;
            AsAtTime = request.QueryDef.AsAtTime.ToDateTimeOffset();
    ...
    internal partial class PackageSubscriptionQuery
    {
        ...
        public PackageSubscriptionQuery(V341CreateSubscription query)
            : base(query)
        {
            SubscriptionId = query.SubscriptionIdGuid;
        }
    }
            ...
            : base(clientId, new PackageHeader(header))
        {
            foreach (V341TransportItem item in body.Items)
            {
                _Items.Add(new CommonItem(
                    item.ItemIdGuid, item.ItemKind.ToItemKind(),
                    ...
                    item.YData.ToByteArray(), item.YSign.ToByteArray(), item.SourceUSN));
            }
        }
    ...
    internal partial class PackageCompletionResult
    {
        ...
        {
            return new V341CompletionResult
            {
                Success = Success,
                Result = Result,
                Message = Message
