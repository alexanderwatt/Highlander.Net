        ...
        ITransferV341    // V3.4 contracts
    {
        ...
        protected override void OnStart()
        {
            ...
            List<CommonItem> subsItems = _cacheEngine.GetCacheItems(
                null, ItemKind.Local, typeof(ClientSubscriptionState).FullName, null, 0, DateTimeOffset.Now, true, false);
        ...
        public List<string> GetServerAddresses(string scheme)
        {
            ...
            //return results;
            throw new NotImplementedException("Removed functionality in gRPC upgrade");
        }
        ...
        public V131SessionReply BeginSessionV131(V131SessionHeader header, V131ClientInfo clientInfo)
        {
            ...
            if (clientInfo.BuildEnv.ToEnvId() < _serverCfg.ModuleInfo.BuildEnv)
            {
                // not valid
                string msg =
            ...
            return new V131SessionReply { SessionId = clientId.ToString() };
        }
        ...
        public void CloseSessionV131(V131SessionHeader header)
        {
            ...
            IConnection connection = _connectionIndex.Remove(header.SessionIdGuid);
            if (connection != null)
            {
                _cacheEngine.DeleteConnectionState(connection.ClientId);
        ...
        {
            if (TryGetValidConnection(header.SessionIdGuid, out var connection))
            {
            ...
            if (TryGetValidConnection(header.SessionIdGuid, out var connection))
            {
                connection.DispatchAsyncRecvCreateSubscription(
                    new PackageCreateSubscription(
                        connection.ClientId, new PackageHeader(header), new PackageSubscriptionQuery(body), body.ExpiryTime.ToDateTimeOffset()));
            }
        }
            ...
            if (TryGetValidConnection(header.SessionIdGuid, out var connection))
            {
                ...
                connection.DispatchAsyncRecvExtendSubscription(
                    new PackageExtendSubscription(
                        connection.ClientId, new PackageHeader(header), body.SubscriptionIdGuid, body.ExpiryTime.ToDateTimeOffset()));
            }
        }
        ...
        public void TransferV341CancelSubscription(V131SessionHeader header, V341CancelSubscription body)
        {
            if (TryGetValidConnection(header.SessionIdGuid, out var connection))
            {
                connection.DispatchAsyncRecvCancelSubscription(
                    new PackageCancelSubscription(connection.ClientId, new PackageHeader(header), body.SubscriptionIdGuid));
            }
        }
