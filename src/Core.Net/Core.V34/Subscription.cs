...
namespace Highlander.Core.V34
{
    ...
    internal class Subscription : ISubscription
    {
        ...
        /// 
        /// </summary>
        public SubscriptionCallback UserCallback { get; set; }
        ...
        public IAsyncResult CancelBegin()
        {
            if (Interlocked.Add(ref _startCount, 0) == 0)
                return null;
            IAsyncResult ar = _client.CancelSubscriptionBegin(Id);
        ...
        internal void ProcessUpdate(ICoreItem item)
        {
            if (Interlocked.Add(ref _startCount, 0) == 0)
            {
                _logger.LogWarning("Subscription '{0}' inactive - ignoring data", Id);
                return;
            }
        ...
        private void NotifyUser(object state)
        {
            ...
            try
            {
                UserCallback?.Invoke(this, item);
            }
            catch (Exception e)
            {
                _logger.LogError("Subscription '{0}' UserCallback() failed: {1}", Id, e);
            }
        }
