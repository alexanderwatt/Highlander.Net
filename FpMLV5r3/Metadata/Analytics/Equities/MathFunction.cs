...
namespace Highlander.Reporting.Analytics.V5r3.Equities 
{
  ...
  public class MathFunction
  {
    ...
    private static double SQRT_PI = 0.56418958354775628695;
    ...
    public double CBND(double a, double b, double rho)
    {
      ...
      double Pi = 3.141592638;
      double res = 0.0;

      if (Math.Abs(rho) >= 1.0) { return res; }

      double a1 = a / System.Math.Sqrt(2 * (1 - rho * rho));
      double b1 = b / System.Math.Sqrt(2 * (1 - rho * rho));
      ...
      if ((a <= 0.0) && (b <= 0.0) && (rho <= 0.0))
      {
        ...
        for (int idx = 0; idx != 5; idx++)
        {
          ...
          {
            sum += x[idx] * x[jdx] * System.Math.Exp(a1 * (2.0 * y[idx] - a1)
