...
using Highlander.Reporting.Analytics.V5r3;
        ...
        {            
            ...
            if (stockASXParent.VolatilitySurface.Expiry[0].Strikes[0].InterpolationModel.GetType() == typeof(WingModelInterpolation))
            {
                ExtrapolationHelper.DoExtrapolation(stockASXParent,
                                      ...
                                      stockSDParent,
                                      child);
                Assert.AreEqual(Convert.ToDouble(child.NodalExpiry[0].Strikes[0].Volatility.Value), 0.295126142, 0.0001);
            }        
        }
        ...
        {        
            ...
            if (stockASXParent.VolatilitySurface.Expiry[0].Strikes[0].InterpolationModel.GetType() == typeof(WingModelInterpolation))
            {
                ExtrapolationHelper.DoExtrapolation(stockASXParent,
                                      ...
                                      child);
                //moneyness = 0.3;
                Assert.AreEqual(Convert.ToDouble(child.NodalExpiry[2].Strikes[0].Volatility.Value), factor * parentVol3, 0.001);
                //moneyness = 1.0;
                Assert.AreEqual(Convert.ToDouble(child.NodalExpiry[2].Strikes[1].Volatility.Value), factor * parentVol1, 0.001);
                //moneyness = 1.2;
                Assert.AreEqual(Convert.ToDouble(child.NodalExpiry[2].Strikes[2].Volatility.Value), factor * parentVol2, 0.001);
            }
        }
