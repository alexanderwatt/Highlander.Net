...
namespace Highlander.Equity.Calculator.V5r3
{
    ...
    {
        private readonly List<EquityStrike> _strikes = new List<EquityStrike>();
        ...
        [XmlArray("Strikes")]
        public EquityStrike[] Strikes
        {
            get
            {
                var strikes = new EquityStrike[_strikes.Count];
                if (_strikes.Count > 0)
                {
        ...
        internal EquityStrike[] NodalStrikes
        {
            get
            {
                var strikeList = new List<EquityStrike>(_strikes);
                List<EquityStrike> nodalStrikes = strikeList.FindAll(item => item.NodalPoint);
        ...
        ///     </code>
        /// </example>
        public void AddStrike(EquityStrike strike, bool nodal)
        {
        ...
        {
            EquityStrike matchedStrike = _strikes.Find(strike => (strike.StrikePrice == strikePrice)
                );
