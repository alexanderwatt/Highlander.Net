...
namespace Highlander.WebAPI.V5r3
{
    ...
    internal static class RangeExtension
    {
        ...
        /// Values as array.
        /// </summary>
        /// <param name="valArray">The value array.</param>
        /// <returns></returns>
        public static object[][] ValueAsArray(this object[,] valArray)
        {
            int lb = valArray.GetLowerBound(0);
            int ub = valArray.GetUpperBound(0);
            int lb1 = valArray.GetLowerBound(1);
            int ub1 = valArray.GetUpperBound(1);
            var values = new object[ub - lb + 1][];
            for (int i = lb; i <= ub; i++)
            {
                values[i - lb] = new object[ub1 - lb1 + 1];
                for (int j = lb1; j <= ub1; j++)
                    values[i - lb][j - lb1] = valArray[i, j];
            }
            return values;
        }
        ...
        /// <returns></returns>
        public static bool ResultOverFillsSelection(this object[,] result, int rowCount, int columnCount)
        {
