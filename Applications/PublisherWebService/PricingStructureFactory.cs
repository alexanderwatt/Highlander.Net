...
namespace Orion.PublisherWebService
{
    internal static class PricingStructureFactory
    {
        ...
        /// </summary>
        /// <returns></returns>
        internal static IPricingStructure CreatePricingStructure(ILogger logger, ICoreCache cache, object[][] propertiesRange, object[][] values, object[][] additional)
        {
            ...
            PricingStructureTypeEnum psType;
            EnumHelper.TryParse(suppliedPricingStructureType, true, out psType);
            if (psType != PricingStructureTypeEnum.RateVolatilityMatrix)
            {
                return CreatePricingStructure(logger, cache, propertiesRange, values);
            }
            ...
            return new RateVolatilitySurface(logger, cache, properties, expiries.ToArray(), vols.ToArray(), inputInstruments.ToArray(),
                inputSwapRates.ToArray(), inputBlackVolRates.ToArray());
        }

        internal static IPricingStructure CreatePricingStructure(ILogger logger, ICoreCache cache, object[][] propertiesRange, object[][] values)
        {
            var properties = propertiesRange.ToNamedValueSet();
            string suppliedPricingStructureType = properties.GetString(CurveProp.PricingStructureType, true);
            PricingStructureTypeEnum psType;
            ...
            {
                return CurveEngine.Factory.PricingStructureFactory.CreatePricingStructure(logger, cache, null, null, properties, ConvertObjectArray(values));
            }
            string[] expiries = ExtractExpiries(values);
            double[,] volatilities = ExtractVols(values, expiries.Length);
            if (psType == PricingStructureTypeEnum.RateATMVolatilityMatrix)
            {
                string[] tenors = ExtractTenors(values);
                return new RateATMVolatilitySurface(logger, cache, properties, expiries, tenors, volatilities);
            }
            if (PricingStructureHelper.VolSurfaceTypes.Contains(psType))
            {
                double[] strikes = ExtractStrikes(values);
                return CurveEngine.Factory.PricingStructureFactory.CreateVolatilitySurface(logger, cache, properties, expiries, strikes, volatilities);
            }
            throw (new Exception(string.Format("{0} cannot be created using this function",
                                               suppliedPricingStructureType)));
        }
        ...
        private static double[] ExtractStrikes(object[][] rawGrid)
        {
            ...
            for (int cols = 1; cols < maxColumns; cols++)
            {
                ...
                double strike;
                if (!double.TryParse(rawGrid[0][cols].ToString(), out strike))
                {
                    throw new InvalidCastException(string.Format("Cannot cast '{0}' to strike", rawGrid[0][cols]));
                }
                strikes.Add(strike);
            }
        ...
        private static double[,] ExtractVols(object[][] rawGrid, int maxRows)
        {
            ...
            for (int column = 1; column <= maxColumns; column++)
            {
                for (int row = 1; row <= maxRows; row++)
                {
                    ...
                    if (!double.TryParse(rawGrid[row][column].ToString(), out vol) || vol < 0)
                    {
                        throw new InvalidCastException(string.Format("Cannot cast '{0}' to volatility", rawGrid[row][column]));
                    }
