...
namespace Orion.Analytics.Tests.Numerics
{
    ...
    public class BlasTests
    {
        ...
        {
            return Math.Min(n, Math.Min(m, TesterUtilities.getInt(numax, _random)));
        }
        ...
        {
            return Math.Min(n, TesterUtilities.getInt(numax, _random));
        }

        [TestMethod]
        public virtual void DenseVector()
        {
            int n = TesterUtilities.getInt(nmax, _random), nu = GetNu(n);
            IElementalAccessVector x = new DenseVector(n);
