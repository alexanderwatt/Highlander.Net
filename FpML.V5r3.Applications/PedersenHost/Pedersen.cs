...
namespace PedersenHost
{
    ...
    public class Pedersen
    {
        ...
        public Pedersen()
        {
            ...
            const string fullAppName = "PedersenHost";
            try
            {
                //var stopwatch = new Stopwatch();
                LogRef = Reference<ILogger>.Create(new TraceLogger(true));
                //stopwatch.Start();
                var factory = new CoreClientFactory(LogRef)
                    .SetEnv("DEV")
            ...
            Engine = new Orion.CurveEngine.CurveEngine(LogRef.Target, _cache, NameSpace);
         ...
         */
        public object SetCurrentDiscount(String curveId)
        {
            const int length = 123;
            object temp;
            try
            {
                ...
                {
                    discount[i] = Engine.GetValue(curveId, today.AddMonths(i * 3));
                }
                _usd.CurDiscount = discount;
                _usd.DiscountStatus = "User Defined";
                result[0, 0] = String.Format("Discount factors were set to ({0}).", curveId);
        ...
        public object SetCurrentCpltIvol(String volId)
        {
            const int length = 123;
            object temp;
            try
            {
                //var range = (Range)r1;
                //string s = range.Value2.ToString();
                temp = Engine.GetCurve(volId, false);
                if (temp.GetType() == typeof(Object[,]))
                {
                    ...
                    result[0, 0] = string.Format("Caplet Implied Vols were set to ({0}).", volId);
                    temp = result;
                }
            }
            catch (Exception e)
        ...
        public object SetCurrentSwpnIvol(String volSurfaceId)
        {
            object temp;
            try
            {
                //var range = (Range)r1;
                //string s = range.Value2.ToString();
                temp = Engine.GetCurve(volSurfaceId, false);
                if (temp.GetType() == typeof(Object[,]))
                {
                    var objects = (object[,])temp;
                    ...
                    _usd.RawSwpnIvol = ivol;
                    _usd.SwpnIvolStatus = "User Defined";
                    result[0, 0] = string.Format("Swaption Implied Vols were set to ({0}).", volSurfaceId);
                    temp = result;
                }
            }
            catch (Exception e)
