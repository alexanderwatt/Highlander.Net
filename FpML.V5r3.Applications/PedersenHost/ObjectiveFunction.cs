...
namespace PedersenHost
{
    class ObjectiveFunction
    {
        ...
        public void Initialise()
        {
            ...
            _evalgrad = false;
            _mc = new MyCounter();
            _xgrad = new GeneralVector(new double[0]);
        ...
        public double ObjFun(Vector x)
        {
            ...
            Fval = temp;
            _itercount++;
            Pedersen.WriteRange(1, 1, String.Format("{0}, {1:f9}, {2}", _itercount, temp, _hit));
            return temp;
        }
        ...
        {
            double total = 0;
            for (int i = 0; i < _param.SwpnExp.Length; i++)
            {
                if (_param.SwpnExp[i] > _param.Uexpiry - 1)
                {
                    break;
                }
                for (int j = 0; j < _param.SwpnTen.Length; j++)
                {
                    if (_param.SwpnExp[i] + _param.SwpnTen[j] > _param.Utenor - 1)
                    {
                        break;
                    }
                    double tempivol = _economy.ReturnIvol(_param.SwpnExp[i], _param.SwpnTen[j]);
                    if (tempivol > 0)
                    {
                        double temp = Math.Log(_economy.FindIvol(_param.SwpnExp[i], _param.SwpnTen[j]) /
                                tempivol);
        ...
        private double QOFSwpn(GeneralMatrix gm, int change)
        {
            ...
            for (int i = 0; i < _param.SwpnExp.Length; i++)
            {
                ...
                for (int j = 0; j < _param.SwpnTen.Length; j++)
                {
                    ...
                    if (tempivol > 0)
                    {
                        double temp = Math.Log(_economy.FindIvol(_param.SwpnExp[i], _param.SwpnTen[j], gm, change) /
                                tempivol);
                        total += temp * temp;
                    }
                }
