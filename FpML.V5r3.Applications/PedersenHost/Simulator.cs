    ...
    class Simulator
    {
        ...
        public double Go()
        {
            ...
            for (int i = 0; i < Iter; i++)
            {
                if (i % 100 == 0)
                {
                    Pedersen.WriteRange(String.Format("Iteration # {0}", i));
                }
            ...
            if (Deriv == Derivative.Caplet)
            {
                Pedersen.Write(String.Format("Caplet: {0}, ", Exp), "sim");
            }
            else if (Deriv == Derivative.Swaption)
            {
                Pedersen.Write(String.Format("Swaption: ({0}, {1}), ", Exp, Ten), "sim");
            }
            if (actual == 0)
            {
                Pedersen.Write(String.Format("Notional: {0}, Iterations: {1}\n", Notional, Iter), "sim");
                Pedersen.Write("Theoretical: N/A\n", "sim");
                Pedersen.Write(String.Format("Sim Average: {0}\n", mean), "sim");
                Pedersen.Write(String.Format("Sim Std.Dev: {0}\n", sd), "sim");
                Pedersen.Write("Error / S.D: N/A\n", "sim");
            }
            else
            {
                Pedersen.Write(String.Format("Notional: {0}, Iterations: {1}\n", Notional, Iter), "sim");
                Pedersen.Write(String.Format("Theoretical: {0}\n", actual), "sim");
                Pedersen.Write(String.Format("Sim Average: {0}\n", mean), "sim");
                Pedersen.Write(String.Format("Sim Std.Dev: {0}\n", sd), "sim");
                Pedersen.Write(String.Format("Error / S.D: {0}\n", error), "sim");
            }
