...
using FileImportRule = Highlander.Metadata.Common.FileImportRule;
using ImportFileResult = Highlander.Metadata.Common.ImportFileResult;
using ImportRuleResult = Highlander.Metadata.Common.ImportRuleResult;
using RuleObject = Highlander.Core.Common.RuleObject;

#endregion

namespace Highlander.Core.FileImporterServer
{
    ...
    public class FileImportServer : ServerBase2
    {
        ...
        private const string AppName = "HIGHLANDER_FileImportServer";
                    ...
                    if (!rule.Disabled)
                    {
                        ...
                        if (Expr.CastTo(rule.CheckConstraint.Evaluate(properties, currentTime), false))
                        {
                            ...
                            if (Expr.CastTo(rule.ImportCondition.Evaluate(properties, currentTime), false))
                            {
                                ...
                                logger.LogInfo("File names to copy: {0}", rule.CopyFilePatterns);
