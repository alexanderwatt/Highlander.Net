    ...
    public abstract class AsyncQueueBase : IDisposable
    {
        ...
        public bool IsIdle
        {
            get
            {
                ...
                try
                {
                    return ((OnGetQueueLength() == 0) && (Interlocked.Add(ref _dequeueThreads, 0) == 0));
                }
                finally
                {
                    ReleaseLock();
        ...
        public long WaitUntilLengthLEQ(long queueLength, TimeSpan timeout)
        {
            ...
            while (((Length > queueLength) || (Interlocked.Add(ref _dequeueThreads, 0) > 0)) && (DateTimeOffset.Now < waitExpires))
            {
                Thread.Sleep(1);
            }
        ...
        protected void EnqueueData<T>(T data, AsyncQueueCallback<T> callback, AsyncQueuePriority priority, string itemKey)
        {
            ...
            try
            {
                OnEnqueueItem(item, priority, itemKey);
            }
            finally
            {
                ReleaseLock();
