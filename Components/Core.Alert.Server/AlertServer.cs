...
using AlertRule = Highlander.Core.Common.AlertRule;
using AlertSignal = Highlander.Core.Common.AlertSignal;
using RuleObject = Highlander.Core.Common.RuleObject;
using UserDetail = Highlander.Core.Common.UserDetail;
using UserGroup = Highlander.Core.Common.UserGroup;

#endregion

namespace Highlander.Core.AlertServer
{
    public class AlertServer : ServerBase2
    {
        ...
        private const string AppName = "HIGHLANDER_AlertServer";
        ...
        private void SendAlertSignalEmail(InternalSignal signal, NamedValueSet props)
        {
            try
            {
                ...
                email.Subject =
                    $"HL Alert {EnvHelper.EnvName(IntClient.Target.ClientInfo.ConfigEnv)}. {signal.RuleName}: {signal.Status}";
        ...
        private void ReportUncaughtError(string method, string ruleName, Exception ex, NamedValueSet debugProps)
        {
            ...
            try
            {
                ...
                var email = new MailMessage
                                {
                                    Subject =
                                        $"HL Alert {EnvHelper.EnvName(IntClient.Target.ClientInfo.ConfigEnv)}. UNHANDLED ERROR"
                                };
                //new
        ...
        private void SendEmail(string smtpHost, MailMessage email, string sender, IEnumerable<string> recipients)
        {
            ...
            email.From = new MailAddress(sender,
                $"HL AlertServer {EnvHelper.EnvName(IntClient.Target.ClientInfo.ConfigEnv)}");
