...
namespace Highlander.Numerics.Options
{
    ...
    public class BlackScholesMertonModel
    {
        ...
        /// <param name="vol">Volatility of the relative price change of the underlying asset S. 
        /// Per cent volatility in units of (year)^(-1/2)</param>
        /// <param name="timeToExpiry">Time in years to the maturity of the option.</param>
        /// <returns>An array of results for Black Scholes.</returns>
        public static double[] CalculateGeneralisedModel(Boolean callFlag, double price, double strike, double timeToExpiry, 
                                                         double rate, double costOfCarry, double vol)
        {
            var result = new double[6];
            var discount1 = callFlag ? Math.Exp((costOfCarry - rate) * timeToExpiry) : Math.Exp(-rate * timeToExpiry);
            var discount2 = callFlag ? Math.Exp(-rate * timeToExpiry) : Math.Exp((costOfCarry - rate) * timeToExpiry);
            if (price != 0 && vol != 0 && timeToExpiry != 0)
            {
                var srt = vol * Math.Sqrt(timeToExpiry);

                var d1 = (Math.Log(price / strike) + (costOfCarry + Math.Pow(vol, 2) / 2) * timeToExpiry) / srt;
                var d2 = d1 - srt;
        ...
        public static double CalculateBSMCallValue(double price, double strike, double timeToExpiry,
                                                   double rate, double costOfCarry, double vol)
        {
            var discount1 = Math.Exp((costOfCarry - rate) * timeToExpiry);
            var discount2 = Math.Exp(-rate * timeToExpiry);
            var srt = vol * Math.Sqrt(timeToExpiry);
            var d1 = (Math.Log(price / strike) + (costOfCarry + Math.Pow(vol, 2) / 2) * timeToExpiry) / srt;
            var d2 = d1 - srt;
        ...
        /// <param name="timeToExpiry">Time in years to the maturity of the option.</param>
        /// <returns>A value for the Black Scholes option.</returns>
        public static double CalculateBSMPutValue(double price, double strike, double timeToExpiry,
                                                  double rate, double costOfCarry, double vol)
        {
            var result = CalculateBSMCallValue(-price, -strike, timeToExpiry, rate, costOfCarry, -vol);
            return result;
        }
        ...
        public static double CalculateBSMSpotCallDelta(double price, double strike, double timeToExpiry,
                                                       double rate, double costOfCarry, double vol)
        {
            var discount1 = Math.Exp((costOfCarry - rate) * timeToExpiry);
            var srt = vol * Math.Sqrt(timeToExpiry);
        ...
        public static double CalculateBSMSpotPutDelta(double price, double strike, double timeToExpiry,
                                                      double rate, double costOfCarry, double vol)
        {
            var result = CalculateBSMSpotCallDelta(-price, -strike, timeToExpiry, rate, costOfCarry, -vol);
            return result;
        }
        ...
        public static double CalculateBSMFutureCallDelta(double price, double strike, double timeToExpiry,
                                                         double rate, double costOfCarry, double vol)
        {
            var result = CalculateBSMSpotCallDelta(price, strike, timeToExpiry, rate, costOfCarry, vol);
            return result * Math.Exp(-costOfCarry * timeToExpiry);
        }
        ...
        public static double CalculateBSMFuturePutDelta(double price, double strike, double timeToExpiry,
                                                        double rate, double costOfCarry, double vol)
        {
            var result = CalculateBSMSpotPutDelta(price, strike, timeToExpiry, rate, costOfCarry, vol);
            return result * Math.Exp(-costOfCarry * timeToExpiry);
        }
    }
}